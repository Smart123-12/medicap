...
const DB = {
    ...
    createUser(user) {
        if (db) {
            db.prepare(`INSERT INTO users (id, name, email, phone, password, role, city, specialty, hospital, qualification, verified, active, createdAt, updatedAt)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
                user.id, user.name, user.email, user.phone, user.password, user.role,
                user.city || '', user.specialty || '', user.hospital || '', user.qualification || '',
                user.verified ? 1 : 0, user.active ? 1 : 0, user.createdAt, user.updatedAt
            );
    ...
    getAllUsers() {
        if (db) {
            return db.prepare('SELECT * FROM users ORDER BY createdAt DESC').all().map(u => {
    ...
    getAllDoctors(filters = {}) {
        if (db) {
            ...
            if (filters.specialty && filters.specialty !== 'All') {
                query += ' AND specialty = ?'; params.push(filters.specialty);
            }
            if (filters.city && filters.city !== 'All') {
                query += ' AND city = ?'; params.push(filters.city);
            }
            if (filters.search) {
            ...
            return db.prepare(query).all(params).map(d => {
                ...
                try { d.languages = JSON.parse(d.languages); } catch(e) { d.languages = []; }
                return d;
            });
        }
        let docs = [...memDb.doctors];
        if (filters.specialty && filters.specialty !== 'All') docs = docs.filter(d => d.specialty === filters.specialty);
        if (filters.city && filters.city !== 'All') docs = docs.filter(d => d.city === filters.city);
        if (filters.search) {
            const q = filters.search.toLowerCase();
            docs = docs.filter(d => d.name.toLowerCase().includes(q) || d.specialty.toLowerCase().includes(q) || d.hospital.toLowerCase().includes(q));
        }
        return docs;
    },
    ...
    findDoctorById(id) {
        if (db) {
            ...
            if (d) { d.available = !!d.available; try { d.languages = JSON.parse(d.languages); } catch(e) { d.languages = []; } }
            return d;
        }
    ...
    createDoctor(doctor) {
        if (db) {
            db.prepare(`INSERT INTO doctors (id, name, specialty, qualification, experience, city, hospital, fee, rating, reviews, available, languages, about, createdAt, addedBy)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
                doctor.id, doctor.name, doctor.specialty, doctor.qualification || '',
                doctor.experience || 0, doctor.city || '', doctor.hospital || '',
                doctor.fee || 500, doctor.rating || 4.5, doctor.reviews || 0,
                doctor.available ? 1 : 0, JSON.stringify(doctor.languages || []),
                doctor.about || '', doctor.createdAt || new Date().toISOString(), doctor.addedBy || 'system'
            );
    ...
    updateDoctor(id, updates) {
        if (db) {
            ...
            const values = [];
            for (const [key, value] of Object.entries(updates)) {
                if (key === 'available') { sets.push(`${key} = ?`); values.push(value ? 1 : 0); }
                else if (key === 'languages') { sets.push(`${key} = ?`); values.push(JSON.stringify(value)); }
                else { sets.push(`${key} = ?`); values.push(value); }
            }
            values.push(id);
    ...
    deleteDoctor(id) {
        if (db) { db.prepare('DELETE FROM doctors WHERE id = ?').run(id); return true; }
        const idx = memDb.doctors.findIndex(d => d.id === id);
        if (idx !== -1) { memDb.doctors.splice(idx, 1); return true; }
        return false;
    },
    ...
    getAppointments(userId, role) {
        if (db) {
            let rows;
            if (role === 'admin') rows = db.prepare('SELECT * FROM appointments ORDER BY date DESC').all();
            else if (role === 'doctor') rows = db.prepare('SELECT * FROM appointments WHERE doctorId = ? ORDER BY date DESC').all(userId);
            else rows = db.prepare('SELECT * FROM appointments WHERE userId = ? ORDER BY date DESC').all(userId);
            return rows;
        }
        let apts;
    ...
    createAppointment(apt) {
        if (db) {
            db.prepare(`INSERT INTO appointments (id, userId, doctorId, patientName, phone, date, time, type, status, notes, createdAt)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
                apt.id, apt.userId, apt.doctorId, apt.patientName || '', apt.phone || '',
                apt.date, apt.time, apt.type, apt.status || 'confirmed', apt.notes || '', apt.createdAt
            );
    ...
    updateAppointmentStatus(id, status, userId, role) {
        if (db) {
            if (role === 'admin' || role === 'doctor') {
                db.prepare('UPDATE appointments SET status = ?, updatedAt = ? WHERE id = ?').run(status, new Date().toISOString(), id);
            } else {
                db.prepare('UPDATE appointments SET status = ?, updatedAt = ? WHERE id = ? AND userId = ?').run(status, new Date().toISOString(), id, userId);
            }
            return db.prepare('SELECT * FROM appointments WHERE id = ?').get(id);
        }
        const apt = memDb.appointments.find(a => a.id === id && (a.userId === userId || role === 'admin' || role === 'doctor'));
        if (apt) { apt.status = status; apt.updatedAt = new Date().toISOString(); }
        return apt;
    },
    ...
    getRecords(userId, role) {
        if (db) {
            ...
            if (role === 'doctor') return db.prepare('SELECT * FROM records WHERE doctorId = ? ORDER BY date DESC').all(userId);
    ...
    createRecord(record) {
        if (db) {
            db.prepare(`INSERT INTO records (id, userId, doctorId, doctorName, date, diagnosis, notes, vitals, medications, tests, createdAt)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
                record.id, record.userId, record.doctorId, record.doctorName || '',
                record.date, record.diagnosis || '', record.notes || '', record.vitals || '',
                JSON.stringify(record.medications || []), JSON.stringify(record.tests || []), record.createdAt
            );
...
async function seedData() {
    ...
    if (!existingAdmin) {
        ...
        DB.createUser({
            id: 'admin-001', name: 'System Admin', email: 'admin@medicap.com',
            phone: '+91 90000 00000', password: hashedPassword, role: 'admin',
            city: 'Ahmedabad', verified: true, active: true,
...
app.post('/api/auth/register', async (req, res) => {
    try {
        ...
        const user = DB.createUser({
            ...
            phone: phone.trim(), password: hashedPassword, role: userRole,
            city: city || '', specialty: userRole === 'doctor' ? (specialty || '') : '',
        ...
        setTimeout(() => { DB.updateUser(user.id, { verified: true }); }, 3000);
...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        if (user.active === false || user.active === 0) return res.status(403).json({ error: 'Account deactivated. Contact admin.' });
...
app.put('/api/admin/users/:id/toggle', authenticateToken, requireRole('admin'), (req, res) => {
    ...
    const updated = DB.updateUser(req.params.id, { active: !user.active && user.active !== 0 ? false : true });
    res.json({ message: `User ${updated.active ? 'activated' : 'deactivated'}.`, user: safeUser(updated) });
});
...
app.post('/api/admin/doctors', authenticateToken, requireRole('admin'), (req, res) => {
    const { name, specialty, qualification, experience, city, hospital, fee, about, languages } = req.body;
    if (!name || !specialty) return res.status(400).json({ error: 'Doctor name and specialty are required.' });
    const doctor = DB.createDoctor({
        id: 'doc-' + uuidv4().slice(0, 8), name, specialty, qualification: qualification || '',
        experience: parseInt(experience) || 0, city: city || '', hospital: hospital || '',
        fee: parseInt(fee) || 500, rating: 4.5, reviews: 0, available: true,
        languages: languages || ['English', 'Hindi'], about: about || '',
        createdAt: new Date().toISOString(), addedBy: req.user.id
    });
...
app.post('/api/admin/patients', authenticateToken, requireRole('admin'), async (req, res) => {
    ...
    const user = DB.createUser({
        id: uuidv4(), name: name.trim(), email: email.toLowerCase().trim(),
        phone: phone.trim(), password: hashed, role: 'patient',
        city: city || '', verified: true, active: true,
        createdAt: new Date().toISOString(), updatedAt: new Date().toISOString()
    });
...
app.post('/api/appointments', authenticateToken, (req, res) => {
    ...
    if (!doctorId || !date || !time || !type) return res.status(400).json({ error: 'Doctor, date, time, and type required.' });
    const doctor = DB.findDoctorById(doctorId);
    if (!doctor) return res.status(404).json({ error: 'Doctor not found.' });

    const apt = DB.createAppointment({
        id: 'apt-' + uuidv4().slice(0, 8), userId: req.user.id, doctorId,
        patientName: patientName || req.user.name, phone: phone || '',
        date, time, type, status: 'confirmed', notes: notes || '',
...
app.post('/api/records', authenticateToken, requireRole('doctor', 'admin'), (req, res) => {
    ...
    const record = DB.createRecord({
        id: 'rec-' + uuidv4().slice(0, 8), userId: patientId, doctorId: req.user.id,
        doctorName: req.user.name, date: new Date().toISOString().split('T')[0],
        diagnosis, notes, vitals, medications: medications || [], tests: tests || [],
        createdAt: new Date().toISOString()
    });
...
app.get('/api/stats', authenticateToken, (req, res) => {
    ...
    res.json({
        ...
        upcomingAppointments: appointments.filter(a => new Date(a.date) >= now && a.status !== 'cancelled').length,
